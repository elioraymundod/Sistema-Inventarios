...
export class BandejaVendedorComponent implements OnInit {
  @ViewChild('mapaCarrito')
  mapDivElement!: ElementRef
  private debounceTimer?: NodeJS.Timeout;
  cantidadNotificaciones: Number;
  dataSource = new MatTableDataSource();
  displayedColumns: string[] = ["fecha", "direccion", "correo", "estado", "accion"]
  dataSourceCarrito = new MatTableDataSource();
  displayedColumnsCarrito: string[] = ['producto', 'cantidad'];
  listaProductos: any[] = [];
  ventaSeleccionada: any;
  showButtonEnviarVenta: boolean = false;
  tiempo: string = '';
  codigoEnvio: string= '';
  correoEnvio: string = '';

  constructor(private ventasService: VentasService, private placesService: PlacesService, private mapService: MapService, private router: Router
  ...
  getVentasPendientes() {
    this.ventasService.getVentasPendientes().subscribe(res => {
      this.cantidadNotificaciones = res.length;
    })
  }
  ...
  setValues(data: any) {
    ...
    this.codigoEnvio  = data.codigo_envio;
    this.correoEnvio = data.correo;

    if (data.venta_atendida === 4) {
      this.showButtonEnviarVenta = true;
    }
    this.ventasService.getDetalleVenta(data.id).subscribe(res => {
      const resultado = res.map((r: any)=> {
        this.listaProductos = [...this.listaProductos, r]
      })
    ...
    const popupDestino = new Popup()
      .setHTML(`
             <h6>Lugar de destino</h6>
         `);
    ...
    new Marker({ color: 'red' })
      .setLngLat(JSON.parse(data.coordenadas_destino))
      .setPopup(popupDestino)
      .addTo(map)


    const start = this.placesService.userLocation!;
    const end = JSON.parse(data.coordenadas_destino);

    this.mapService.setMap(map);
    if(this.debounceTimer) clearTimeout(this.debounceTimer);
    this.debounceTimer = setTimeout(() => {
      this.mapService.getRouteBetwenPoints(start, end, false)
    }, 1000);
  }
  ...
  enviarPedido(){
    ...
    this.ventasService.updateEstadoVenta(venta).subscribe(res => {
      Swal.fire({
        icon: 'success',
    ...
    this.enviarCorreoEnvioProducto(this.correoEnvio);
    this.getVentasPendientes();
    this.getAllVentas();
  }

  enviarCorreoEnvioProducto(email: any){
    const datosCorreo = {
      paraCorreo: email,
      asuntoCorreo: 'Notificacion de envio de pedido a domicilio',
      cuerpoCorreo: '',
      htmlCorreo: `
 ...
 <body data-new-gr-c-s-loaded="14.1085.0" style="width:100%;font-family:arial, 'helvetica neue', helvetica, sans-serif;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;padding:0;Margin:0">
  <div class="es-wrapper-color" style="background-color:#CEEAF4"><!--[if gte mso 9]>
   ...
   <table class="es-wrapper" width="100%" cellspacing="0" cellpadding="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;padding:0;Margin:0;width:100%;height:100%;background-repeat:repeat;background-position:center top;background-color:#CEEAF4">
               ...
               <table cellpadding="0" cellspacing="0" width="100%" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                 <tr>
                  <td align="center" valign="top" style="padding:0;Margin:0;width:560px">
                   <table cellpadding="0" cellspacing="0" width="100%" role="presentation" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px">
                     <tr>
                      <td align="center" class="es-m-txt-c es-m-p0r es-m-p0l" style="padding:0;Margin:0;padding-left:40px;padding-right:40px"><h1 style="Margin:0;line-height:36px;mso-line-height-rule:exactly;font-family:Comic Sans Ms, sans-serif;font-size:26px;font-style:normal;font-weight:bold;color:#455A64">Le informamos que su pedido ha sido enviado al domicilio indicado, a partir de este momento el pedido llegará en aproximadamente ${this.tiempo}. Al llegar el asesor por favor proporcionar el código  "${this.codigoEnvio}" para finalizar la entrega.</h1></td>
                     </tr>
                     <tr>
    ...
    this.correoService.enviarCorreo(datosCorreo).subscribe(res => {})
  }
